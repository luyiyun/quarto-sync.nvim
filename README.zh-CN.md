...
`quarto-sync.nvim` 是一个 Neovim 插件，同时内置一个 Quarto filter extension，用于把 `.qmd` 源文件中的光标位置单向同步到浏览器里的 Quarto HTML 预览页面。

插件会启动 `quarto preview`、启动一个本地同步服务，并监听 Neovim 中的光标移动。`:QSyncPreview` 会渲染一个临时的 shadow `.qmd` 文件，在其中插入不可见的源码行标记，然后浏览器端脚本根据这些标记滚动到对应位置。
...
- `:QSyncStatus`：显示 preview、server、port、原始文件、shadow 文件、预览 URL、最近同步行和最近识别到的 anchor。
...
4. 修改内容后保存文件，shadow 文件会在 `BufWritePost` 自动重新生成。
